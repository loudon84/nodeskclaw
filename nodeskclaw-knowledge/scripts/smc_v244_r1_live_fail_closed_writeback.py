...
def main() -> int:
    try:
        ...
        if str(os.environ.get("KNOWLEDGE_LIVE_FAULT_SLICE_FAIL") or "").strip() != "1":
            raise RuntimeError("fault driver KNOWLEDGE_LIVE_FAULT_SLICE_FAIL=1 required")
        ...
        status, payload = http_json(
            ...
            headers=headers,
            body={"query": "smc v244 r1 fail closed", "channel": "stable"},
        )
        ...
        if (
            status == 503
            and message_key_of(payload) == "errors.knowledge.retrieval_unavailable"
            and chunk
            and chunk.get("retrieval_status") == "unavailable"
        ):
            results["CLM-08"] = "PASS"
            results["CLM-11"] = "PASS"
        emit_result(results)
