...
def public_ok(payload: dict[str, Any]) -> bool:
    ...
    return bool(payload.get("evidence_id"))
...
def main() -> int:
    try:
        ...
        v2_status, v2_payload = http_json(
            ...
            headers=headers,
            body={"query": "smc v244 r1 retrieval", "channel": "stable"},
        )
        agent_status, agent_payload = http_json(
            "POST",
            f"{know}/api/v2/agent/tools/knowledge.search",
            headers=headers,
            body={"query": "smc v244 r1 retrieval", "application_id": application_id, "channel": "stable"},
        )
        results = fail_all()
        ...
        if (
            ...
            and public_ok(v2_data)
            and public_ok(agent_data)
            and not leaks_provider_ids(v2_data)
            and not leaks_provider_ids(agent_data)
        ):
            results["CLM-04"] = "PASS"
            results["CLM-11"] = "PASS"
        emit_result(results)
