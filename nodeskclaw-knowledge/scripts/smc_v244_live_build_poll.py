...
def main() -> int:
    try:
        ...
        if not job_id:
            ...
            job_id = unwrap(validate_payload).get("validation_job_id")
            if validate_status >= 400 or not job_id:
                emit_result(fail_all())
                return 1
        status, payload = http_json("GET", f"{know}/api/v2/builds/{job_id}", headers=headers)
        results = fail_all()
        if status == 200 and unwrap(payload).get("id") == job_id:
            results["CLM-05"] = "PASS"
            results["CLM-15"] = "PASS"
        emit_result(results)
