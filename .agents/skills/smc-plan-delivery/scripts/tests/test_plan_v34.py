...
''', encoding="utf-8")
    ...
    def test_migration_leaves_v35_unchanged(self):
        # @lat: [[agent-skills-governance#Agent Skills Governance#Plan Closure Contract#Current Delivery Contracts#Legacy migrate leaves v3.5 unchanged]]
        text = LEGACY.replace("plan_contract: smc.plan.v3.3", "plan_contract: smc.plan.v3.5")
        self.plan.write_text(text, encoding="utf-8")
        script = HERE / "migrate_legacy_plan.py"
        result = subprocess.run([sys.executable, str(script), str(self.plan), "--in-place"], capture_output=True, text=True)
        self.assertEqual(0, result.returncode, result.stdout + result.stderr)
        self.assertIn("PLAN_ALREADY_CURRENT: smc.plan.v3.5", result.stdout)
        self.assertEqual("smc.plan.v3.5", common.parse_top_level_frontmatter(self.plan.read_text(encoding="utf-8"))["plan_contract"])
