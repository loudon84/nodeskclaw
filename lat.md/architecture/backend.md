...
**Rich Runtime Events Owner**：既有 Agent Event SoT + StoragePort 与 Backend Public Projection；合同 `v1.6.0` 已发布（tag `skill-run-contract-v1.6.0`）。Roadmap RM-20 仍 `IN_PRD`，live REAL_PROCESS 未闭环。见 [[architecture/skill-agent#RM-20 Public Rich Runtime Events]]。
