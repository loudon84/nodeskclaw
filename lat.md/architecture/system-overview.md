...
软删除与 Partial Unique Index 是全库不变量（[[decisions/soft-delete]]）。Backend 共享文件经 `storage_service`（S3 + 本地双后端）；Skill Agent 工件经独立 [[nodeskclaw-agent/app/services/storage_port.py#StoragePort]]（local 或真实 S3 兼容后端，验收拓扑用 MinIO），不复用 Backend `storage_service`。RM-04 验收 Compose 只作验证拓扑：双 Central、单 Edge、MinIO 与 Native `hermes-test`，不把 Docker 写进生产 Adapter，见 [[architecture/skill-agent#Production Readiness And Security#Native Acceptance Fixture]]。AutoTask 自有库含 `task_successor_jobs` 与 `rpa_runs.output`（迁移 `7c1f4d8e2a90`，见 [[task#Schema Migration Successor]]）。
