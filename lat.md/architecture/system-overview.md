...
NoDeskClaw 由 Portal、Backend、LLM Proxy、Knowledge、Task、运行时 Provider 与 Channel 插件组成；Backend 是 API 与编排中枢。

请求从 Portal / Admin / Desktop / MCP Client 进入双前缀 API，经 services 编排后落到 PostgreSQL、K8s/Docker 与外部 GeneHub / Provider / RAGFlow。组件边界见 [[lat#Component Boundaries]]。AutoTask 独立服务见 [[task]]。
...
3. **MCP / Expert**：JSON-RPC MCP Gateway（组织 MCP + Expert MCP）；apps/work 消费面冻结为 [[decisions/work-expert-contract|WORK-EXPERT-CONTRACT v1.0.0]]。
