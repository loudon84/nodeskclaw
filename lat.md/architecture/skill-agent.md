...
- **部分实现**：Stage PRD `docs_agent/prd-v1.6.14-hermes-provider-conformance-recovery.md` 已 APPROVED；P0 修正子 PRD `reports/PRD-RM16-P0-Live-Conformance-Correction-v1.6.14-p0.1.md` 已 APPROVED。canonical Plan 为 `.cursor/plans/rm-16_hermes-provider-conformance-recovery.plan.md`。Adapter / runner P0 已落地，含 SSE `event:` ingest、GET `output` 单条回填、有界 `source_event_id`，以及 coalescer 1s 窗口 / 流式 `assistant.message` / 快照去重 / 短段落缓冲，见 [[architecture/skill-agent#Hermes Engine Adapter#Runtime Semantic Event Fidelity]]。live 套件见 [[architecture/skill-agent#RM-16 Live Conformance]]。生产 Native Bridge / 审批驻留仍见 [[architecture/skill-agent#RM-15 Approval Runtime Control]]。
...
- **部分实现**：全套重跑证据（`2026-09-06T11:02Z`–`11:04Z`）为：PC-01 / PC-06 FAIL（流式碎块与全文快照重复，Adapter coalescer 修复尚未 live 验证）；PC-02 / PC-04 / PC-07 / PC-12 PASS；PC-03 / PC-05 / PC-08 BLOCKED；PC-09 FAIL。`chat_completions_observed=false`，`runtime_binding_verified=true`。不得用旧 `10:04Z` PASS 覆盖当前 FAIL。RM-02 Revalidation Link 只引用本套件，不得在 implementation commit 里改写 RM-02 Roadmap 状态。
...
- **部分实现**：RM-16 生产路径已补 `/approval` 接受条件、cancel 合同终态、Worker gap、最小 `internal.runtime.trace` 与 coalescer 合并规则，见 [[architecture/skill-agent#RM-16 Provider Conformance Grounding]] 与 [[architecture/skill-agent#Hermes Engine Adapter#Runtime Semantic Event Fidelity]]。live 证据不得把 PC-01 / PC-06 记为 PASS，见 [[architecture/skill-agent#RM-16 Live Conformance]]。
