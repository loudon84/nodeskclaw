...
RM-19 在不改写 v1.2.1～v1.4.0 的前提下发布累积 Public Streaming Delta 合同。

- **已实现**：Agent 消息段生命周期 `OPEN -> assistant.delta* -> assistant.message -> CLOSED`；Backend allowlist 投影；`scripts/contracts.py` 支持 `--version 1.5.0`；两提交发布（行为 commit A + Bundle-only commit B + tag `skill-run-contract-v1.5.0`）。
- **目标状态**：真实 `user_jwt` REAL_PROCESS 证明运行中 delta（非 terminal 批量回放）；仓外 Work UI/consumer-lock 不是本仓 DONE。
