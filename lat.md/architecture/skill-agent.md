...
- **部分实现**：live 全套 REAL_PROCESS 证据尚未关闭。Adapter 已立刻订 `/events`，解析 SSE `event:` 行，并在流上无助手文本时用完成态 GET `output` 回填 PC-01 / PC-06。PC-07 仍要求真实 `subagent.*` SSE。已部署 Skill Agent 必须加载本 Adapter 后再跑 live。RM-02 Revalidation Link 只引用本套件，不得在 implementation commit 里改写 RM-02 Roadmap 状态。
