...
- **部分实现**：live runner [[tools/acceptance/run_rm18_live_attachment.py#run_live]] 已落地；无 Catalog `supportsAttachments=true` Skill 时记阻断、不猜生产元数据。annotated tag `skill-run-contract-v1.4.0` 须在 implementation commit 之后创建，禁止 `git tag -f`。Roadmap RM-18 仍 `IN_PRD`。
- **目标状态**：REAL_PROCESS `user_jwt` live `SMC_ACCEPTANCE_RESULT` 全 PASS；tag + `check --release` PASS；独立 Roadmap DONE。仓外 Work UI、Agent 读字节 / Hermes 注入不在本项。
