...
- **已实现**：PC-07 要求 Agent SoT 出现最小 `internal.runtime.trace`（生产路径见 [[architecture/skill-agent#Hermes Engine Adapter#Runtime Semantic Event Fidelity]]）；Public `_public_run_event` 对该类型返回 `None`。缺 `LIVE_SUBAGENT_TOOL_NAME` / `RM16_SUBAGENT_TOOL_NAME` 记 `BLOCKED` / `RM16_SUBAGENT_FIXTURE_UNAVAILABLE`，无真实 delegation 不得 PASS。
...
- **部分实现**：live 全套 REAL_PROCESS 证据尚未关闭。Adapter 已改为 `POST /v1/runs` 后立刻订 `/events`，再 persist Binding。部署新 Agent 后需重跑 PC-01 / PC-06 / PC-07。RM-02 Revalidation Link 只引用本套件，不得在 implementation commit 里改写 RM-02 Roadmap 状态。
