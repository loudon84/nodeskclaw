...
RM-16 仍为 Roadmap BACKLOG。live 出口不含 PC-05 / PC-08；Worker fencing 与 interrupted 走单测，禁止再跑 Worker kill 与 Hermes restart。
...
- **目标状态**：PC-01、PC-02、PC-03、PC-04、PC-06、PC-07、PC-09 全部 REAL_PROCESS 后才能关闭本项；禁止再测 PC-05 / PC-08 live；禁止 mock OpenAI 字段结项；不得改写 v1.2.1，不得恢复 ChatCompletion parser。Roadmap 现为 `BACKLOG`，不得标 `DONE`。
...
- **已实现**：2026-09-08 REAL_PROCESS JSON 已跟踪为 `docs_agent/evidence/RM-16-live-*.json`（pc01/pc02/pc03_approve/pc03_deny/pc04/pc06/pc07/pc09/pc12_scan 与 rm02-package 均为 PASS）。PC-03 Native `/approval` 接受。`chat_completions_observed=false`，`runtime_binding_verified=true`。历史 pc05/pc08 `BLOCKED` 文件不是出口。RM-02 继续 `BACKLOG`，Revalidation Link 只引用本套件（不含 pc05/pc08 live），不得把 RM-02 标 `DONE`。
- **目标状态**：Roadmap Status 仍为 `BACKLOG`，须独立 `DONE` commit；`--scenario pc05` / `pc08` 保持拒绝。
