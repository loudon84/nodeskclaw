...
- **部分实现**：live 证据 `docs_agent/evidence/RM-16-live-pc01.json` / `RM-16-live-pc06.json`（`2026-09-06T10:04Z`）为 PASS；`RM-16-live-pc02.json` / `RM-16-live-pc07.json`（`2026-09-06T10:27Z`）FAIL。后一次 Agent SoT 停在 `RUNTIME_RUNNING`，`runs.result` 为 `source_event_id` `varchar(128)` 截断。Adapter 已改为 `hermes:{attempt_id}:{counter}` 且列宽 512，须部署并跑 Alembic 后再重跑 PC-02 / PC-07。RM-02 Revalidation Link 只引用本套件，不得在 implementation commit 里改写 RM-02 Roadmap 状态。
