...
Hermes Connectors（`/hermes/connectors`）与 Edge 节点（`/hermes/edge-nodes`）运营页管理 Connector 定义/实例/公开 Tool、SecretRef 元数据与 Edge 登记；Edge 登记返回一次性 bootstrap（含过期时间）并支持 disable/enable/rotate/revoke。运营者经 [[nodeskclaw-portal/src/api/hermes/connectors.ts#rotateEdgeNode]] 只开启窗口：[[nodeskclaw-portal/src/api/hermes/connectors.ts#EdgeNode]] 的 `identity_rotation_expires_at` 未过期时，`EdgeNodesView` 显示 `rotating` 徽标与轮换截止列，期间轮换按钮禁用并提示等待 Edge Agent 绑定；绑定在 Edge Agent 本地完成，Portal 不收集密钥明文。文案键：`hermes.edgeNodes.colRotationExpires`、`hermes.edgeNodes.rotatePendingHint`、`hermes.edgeNodes.status.rotating`（`zh-CN` / `en-US`）。页面 `nodeskclaw-portal/src/views/hermes/EdgeNodesView.vue`，API [[nodeskclaw-portal/src/api/hermes/connectors.ts#createEdgeNode]] / [[nodeskclaw-portal/src/api/hermes/connectors.ts#disableEdgeNode]] / [[nodeskclaw-portal/src/api/hermes/connectors.ts#enableEdgeNode]] / [[nodeskclaw-portal/src/api/hermes/connectors.ts#revokeEdgeNode]]。
