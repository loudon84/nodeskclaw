...
- **Edge Control Channel Closure（RM-07）**：Internal Edge 出站请求必须携带 Ed25519 证明并通过 append-only [[nodeskclaw-backend/app/models/connector/edge_control_nonce.py#EdgeControlNonce]] 反重放；Backend 对 job claim、install desired、on-demand 与 cancel check 下发 issuer 签名的 `{envelope, payload}` 封套，Agent 验签与本地 command consume ledger 通过后才产生 Connector/安装/Artifact/cancel 副作用；一次性 bootstrap 仅用于 enroll，不得作为长期 Token；命令封套不进入 Public Consumer Contract。
