...
- RM-14：[Runtime Semantic Event Fidelity](../../docs_agent/prd-v1.6.12-runtime-semantic-event-fidelity.md)（PRD `APPROVED`；Roadmap `BACKLOG` 直至 V13）。本地 Normalizer / Coalescer / `phase` 投影已落地，见 [[architecture/skill-agent#Hermes Engine Adapter#Runtime Semantic Event Fidelity]]。Canonical Plan：`.cursor/plans/rm-14_runtime-semantic-event-fidelity.plan.md`（`plan_id: RM-14`，`commit_policy: post_review`）。依赖 RM-13。不得恢复 ChatCompletion parser，不得改写 v1.2.1。
