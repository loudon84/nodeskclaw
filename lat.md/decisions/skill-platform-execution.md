...
- RM-12：[冻结 v1.2.1 员工 Public 面符合性](../../docs_agent/prd-v1.6.10-skill-run-v121-public-conformance.md)（PRD `APPROVED`；Roadmap `DONE`）。出口证据来自 REAL_PROCESS live runner（[[architecture/skill-agent#RM-12 Live Public Conformance]]），`docs_agent/evidence/RM-12-live-conformance.json` 为 `result=PASS`。Canonical Plan：`.cursor/plans/rm-12_v121_public_conformance.plan.md`（`plan_id: RM-12`，`commit_policy: post_review`）。依赖已完成的 RM-06 与 RM-11。不得并入 RM-04 / RM-09；不得改写冻结合同。用户草稿 `docs_agent/prd-hotfix-skill-run-v1.2.1-postman-ready.md` 不是 Stage PRD。RM-09 仍 `BACKLOG`，等待 RM-08 `DONE` 后承接 v1.2.1 **之后**的批准增量。下游只走 `smc-plan-delivery`。
- RM-13：[Hermes Native Runtime Bridge](../../docs_agent/prd-v1.6.11-hermes-native-runtime-bridge.md)（PRD `APPROVED`；Roadmap `DONE`）。C01–C09 在 `59ebfb66` 落地；V11 REAL_RUNTIME 证据见 [[architecture/skill-agent#RM-13 Live Native V11]]。Canonical Plan：`.cursor/plans/rm-13_hermes-native-runtime-bridge.plan.md`（`plan_id: RM-13`，`commit_policy: post_review`）。不依赖 RM-12。不得删除 HermesTaskWorker，不得借用 Knowledge RuntimeBinding。
- RM-14：[Runtime Semantic Event Fidelity](../../docs_agent/prd-v1.6.12-runtime-semantic-event-fidelity.md)（PRD `APPROVED`；V13 live Native `PASS`）。Normalizer / Coalescer / `phase` 投影见 [[architecture/skill-agent#Hermes Engine Adapter#Runtime Semantic Event Fidelity]]；live 出口 [[architecture/skill-agent#RM-14 Live Semantic V13]]。Canonical Plan：`.cursor/plans/rm-14_runtime-semantic-event-fidelity.plan.md`（`plan_id: RM-14`，`commit_policy: post_review`）。依赖 RM-13。不得恢复 ChatCompletion parser，不得改写 v1.2.1。Roadmap status 由独立 commit 关闭。
- RM-15：[Approval Runtime Control Closure](../../docs_agent/prd-v1.6.13-approval-runtime-control-closure.md)（PRD `APPROVED`；Roadmap `DONE`；V13 live Native `PASS`）。Public 批准/拒绝与取消接到同一 Attempt 的 `/approval` `/stop`，见 [[architecture/skill-agent#RM-15 Approval Runtime Control]]；live 出口 [[architecture/skill-agent#RM-15 Live Control V13]]。Canonical Plan：`.cursor/plans/rm-15_approval-runtime-control-closure.plan.md`（`plan_id: RM-15`，`commit_policy: post_review`）。依赖 RM-14。不得改写 v1.2.1，不得恢复 ChatCompletion parser，不得并入 PC-01 至 PC-09。证据提交 `c4210717`。
- RM-16：[Hermes Provider Conformance Recovery](../../docs_agent/prd-v1.6.14-hermes-provider-conformance-recovery.md)（PRD `APPROVED`；Roadmap 未 DONE）。生产路径已补 generation 回落、租约控制头、cancel 合同终态与 Worker gap，见 [[architecture/skill-agent#RM-16 Provider Conformance Grounding]]；live 套件 [[architecture/skill-agent#RM-16 Live Conformance]]。Canonical Plan：`.cursor/plans/rm-16_hermes-provider-conformance-recovery.plan.md`（`plan_id: RM-16`，`commit_policy: post_review`）。依赖 RM-15。不得改写 v1.2.1，不得恢复 ChatCompletion parser，不得把 Backend 做成员工 Native 客户端。
...
- **Cancel/Resume/Approval State Machine**：取消请求支持 `CANCELLING` 中间态与 `cancel_event` 异步中断；绑定 `WAITING_APPROVAL` 同样 `CANCELLING` 后 Hermes `/stop`，再 GET reconcile 落到合同终态，见 [[architecture/skill-agent#RM-16 Provider Conformance Grounding]]；`resume_run` 仅处理 `PAUSED`/`SUSPENDED` 并显式拒绝 `WAITING_APPROVAL`；[[nodeskclaw-agent/app/services/run_service.py#approve_run]] 有 Binding 时回写 Hermes `/approval`（不得 `QUEUED`），无 Binding 时 deny 记 `FAILED`，见 [[architecture/skill-agent#RM-15 Approval Runtime Control]]。
...
- **Hermes 实时 Adapter**：[[nodeskclaw-agent/app/services/hermes_engine.py#execute_hermes_run]] 生产南向为 Native Run（`GET /v1/capabilities`、`POST /v1/runs`、Attempt Binding、`GET /events`）；`waiting_for_approval` 驻留当前 Attempt；批准走 `/approval`，取消走 `/stop` 再 GET reconcile。ChatCompletion `choices` 不是 Event Source。`/events` 经 [[nodeskclaw-agent/app/services/native_event_normalizer.py#normalize_native_event]] 与 [[nodeskclaw-agent/app/services/assistant_delta_coalescer.py#AssistantDeltaCoalescer]] 进入 Agent Event SoT。无 gateway 才 stub。语义保真见 [[architecture/skill-agent#Hermes Engine Adapter#Runtime Semantic Event Fidelity]]；控制面见 [[architecture/skill-agent#RM-15 Approval Runtime Control]] 与 [[architecture/skill-agent#RM-16 Provider Conformance Grounding]]。
...
- **Run 投影**：Backend [[nodeskclaw-backend/app/api/runs.py#get_run]] `/api/v1/runs/*` 鉴权反代 Agent（含 SSE）；员工可见 JSON **剥离** `gateway_url` / token；不对员工开放 Run 创建。POST 体必须经 `json_body` 转发；Agent 4xx 由 [[nodeskclaw-backend/app/api/runs.py#_handle_agent_error_response]] 映射为公共 `error_code` / `message_key` / HTTP 状态，404 仍为 Run 不存在。Public cancel 对 Agent HTTP 5xx 映射为 409，见 [[architecture/skill-agent#RM-16 Provider Conformance Grounding]]。配置：`SKILL_AGENT_BASE_URL` / `SKILL_AGENT_INTERNAL_TOKEN` / `SKILL_AGENT_ENABLED`；禁止复用 `AGENT_API_BASE_URL`。
...
在 `SKILL_AGENT_ENABLED` 时调 Agent `POST /internal/v1/runs`，再写 HermesTask 投影并标记 `execution_owner=agent`；关闭时回退 `execution_owner=backend`（仅过渡）。[[nodeskclaw-backend/app/services/hermes_skill/hermes_task_worker.py#HermesTaskWorker]] 跳过 agent-owned 任务。高风险 Run 可在 create-time 停在 `WAITING_APPROVAL`，批准前不进引擎。Hermes Native `waiting_for_approval` 发生在已绑定 Attempt 上，见 [[architecture/skill-agent#RM-15 Approval Runtime Control]]。`register-to-org-mcp` 只建 draft Release，不自动 published。
...
v1.2.0 增加结构化语义 Run Event 合同（六类语义 + KEEP 控制事件回放），Schema 见 [[nodeskclaw-backend/app/schemas/skill_run/mcp_jsonrpc.py#RUN_EVENT_V12_MODELS]] 与常量 [[nodeskclaw-backend/app/schemas/skill_run/constants.py#SKILL_RUN_CONTRACT_VERSION_V12]]；v1.1.0 目录与 checksum 冻结不改写。当前员工面以冻结 `v1.2.1` 为准。Backend SSE `GET /api/v1/runs/{run_id}/events` 经 [[nodeskclaw-backend/app/api/runs.py#_public_run_event]] 投影控制事件与全部合同语义类型（`assistant.message`、`reasoning.summary`、`tool.call`、`clarify.requested`、`approval.requested`、`artifact.persisted`）；未知事件丢弃。控制事件以 `payload.phase` 为事实字段并派生 `stage`，不再用 HermesTask `status` 猜测 phase。`correlation_confidence` 与 Runtime Binding 键不进 Public，含 `WAITING_APPROVAL` 与 `approval.requested`。Public 审批 mutation 只接受 `decision=approve|deny`；客户端 `session`/`always` 返回 `message_key=errors.run.approval_choice_forbidden`，不改写 v1.2.1 schema，见 [[architecture/skill-agent#RM-15 Approval Runtime Control]]。
