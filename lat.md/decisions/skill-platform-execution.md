...
- RM-12：[冻结 v1.2.1 员工 Public 面符合性](../../docs_agent/prd-v1.6.10-skill-run-v121-public-conformance.md)（`APPROVED`；Roadmap `IN_PRD`）。依赖已完成的 RM-06 与 RM-11。不得并入 RM-04 / RM-09；不得改写冻结合同。用户草稿 `docs_agent/prd-hotfix-skill-run-v1.2.1-postman-ready.md` 不是 Stage PRD。RM-09 仍 `BACKLOG`，等待 RM-08 `DONE` 后承接 v1.2.1 **之后**的批准增量。
