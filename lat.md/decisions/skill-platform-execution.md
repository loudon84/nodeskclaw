...
- **Hermes 实时 Adapter**：[[nodeskclaw-agent/app/services/hermes_engine.py#execute_hermes_run]] 生产南向为 Native Run（`GET /v1/capabilities`、`POST /v1/runs`、Attempt Binding、`GET /events`）；ChatCompletion `choices` 不是 Event Source。`/events` 经 [[nodeskclaw-agent/app/services/native_event_normalizer.py#normalize_native_event]] 与 [[nodeskclaw-agent/app/services/assistant_delta_coalescer.py#AssistantDeltaCoalescer]] 进入 Agent Event SoT。无 gateway 才 stub。语义保真见 [[architecture/skill-agent#Hermes Engine Adapter#Runtime Semantic Event Fidelity]]。
