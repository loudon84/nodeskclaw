...
- RM-12：员工 Public Skill Run 对冻结 `SKILL-RUN-CONTRACT v1.2.1` 可观察符合（Roadmap `READY`；依赖已完成的 RM-06 与 RM-11；Stage PRD 尚未挂接）。不得并入 RM-04 / RM-09；不得改写冻结合同。RM-09 仍 `BACKLOG`，等待 RM-08 `DONE` 后承接 v1.2.1 **之后**的批准增量。
