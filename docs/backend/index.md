...
1. **`tools/list`**：Registry 工具 + `McpToolMapper.list_tools()`（需 `is_mcp_exposed`、已安装、授权可见）
2. **`tools/call`**：`OutputPolicyService.resolve()` → 写入 `output_policy`；创建 `HermesTask`；返回 `task_id`、`artifact_mode=pull_only`、`server_artifacts=[]`
3. **Worker 执行**：全文 `content_text` 物化（非 500 字 `result_summary`）；写 `task.server_artifacts`
4. **结果获取**：轮询 `GET /api/v1/hermes/tasks/{id}/result` 或 `.../artifacts`（含 `server_artifacts`）
5. **预览/下载**：`GET /api/v1/hermes/artifacts/{id}/preview|download`（`object_store` 走 `storage_service`）
...
- PRD：`docs_prd/team_v5.*.md`（Hermes MCP、Runtime Skill 注册、Artifact Bridge 等版本需求）
