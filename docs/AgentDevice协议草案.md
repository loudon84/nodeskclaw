...
| 结果语义 | MUST | 说明动作是成功、失败、被拒绝、等待中、部分完成、不支持，还是需要介入 |
| 状态变化 | MUST | 说明设备状态、租约、当前操作者、现场引用或其他关键状态是否发生变化 |
...
| 能力协商 | MUST | 当目标平台不支持某个能力时，说明缺失能力、替代能力或需要的 Device Profile 能力声明 |
...
| `visibility` | Core Protocol | 设备状态、租约和当前操作者必须以结构化状态可见 | Level 1 / Level 2 |
...
- Core Protocol 的能力协商和语义契约的机器可测试化。
- 通用 Preflight 字段。
