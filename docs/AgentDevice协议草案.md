...
| 是否定义完整状态转换表 | 不定义。v1 规定核心状态、动作和状态约束；具体状态转移路径与失败落点由实现或 Device Profile 约束，并通过 Agent-readable semantic contract 表达状态变化和失败语义 |
...
- Core Protocol 的事件语义、能力协商和语义契约的机器可测试化。
- 通用 Preflight 字段。
