...
设备动作包括 Core Protocol 的基础动作、Control Transfer Profile 的接手动作，以及 Device Profile 中可能产生外部副作用的私有动作。无论目标平台通过自然语言、CLI 输出、Tool 结果、MCP 返回、HTTP API 响应还是 SDK 返回实现，都 MUST 表达以下语义：

| 语义 | v1 要求 |
|------|---------|
| 操作意图 | 说明本次动作试图对哪个 `device_instance_id`、`lease_id`、会话或外部对象做什么 |
| 结果语义 | 说明动作是成功、失败、被拒绝、等待中、部分完成、不支持，还是需要介入 |
| 状态变化 | 说明设备状态、租约、当前操作者、现场引用或其他关键状态是否发生变化 |
| 失败原因 | 失败或拒绝时，说明 Agent 可采取行动的原因，见 2.3 |
| 外部副作用 | 说明是否没有副作用、已经产生副作用、只产生部分副作用，或副作用状态未知 |
| 审计证据 | 说明相关 Audit（审计）事件、`evidence_refs` 或 `observable_surface_ref` 是否存在；产生外部副作用时不得只返回空泛成功 |
| 下一步建议 | 说明 Agent 应继续、停止、重试、刷新状态、释放设备、续租、请求介入或等待 Operator |
| 能力协商 | 当目标平台不支持某个能力时，说明缺失能力、替代能力或需要的 Device Profile 能力声明 |
...
v1 不定义统一 error code（错误码）、error enum（错误枚举）或错误返回结构。失败语义的目的不是让目标平台重写自己的错误体系，而是让 Agent 和 Context Controller 能判断下一步是否安全。

当设备动作不是完全成功时，实现 MUST 表达以下最小决策维度：

| 决策维度 | v1 要求 |
|----------|---------|
| 执行状态 | 说明动作未执行、已执行、部分执行、等待中，还是执行状态未知 |
| 副作用状态 | 说明外部副作用未发生、已发生、部分发生，还是无法确认 |
| 安全重试 | 说明 Agent 是否可以安全重试，或必须先刷新状态、重新申请租约、请求授权、请求介入 |
| 失败类别 | 说明失败大致属于权限/准入、租约/占用、状态冲突、能力不支持、风险策略、外部系统失败、审计或证据不可确认、副作用状态未知 |
| 审计可靠性 | 说明是否已有可引用的 Audit（审计）事件或证据；如果审计不可确认，不能当作普通失败处理 |
| 下一步动作 | 说明 Agent 应停止、重试、刷新状态、释放设备、续租、重新申请租约、请求授权或请求 Operator 介入 |
...
5. 设备动作结果能表达操作意图、结果语义、状态变化、失败原因、外部副作用、审计证据和下一步建议；失败、拒绝、部分完成或状态未知时能表达最小失败语义。
...
- 失败、拒绝、部分完成或状态未知时，必须表达最小失败语义，不要求统一错误码。
...
| 是否定义统一错误模型 | 不定义统一错误码或错误返回结构。v1 只要求最小失败语义，确保 Agent 能判断执行状态、副作用状态、安全重试、失败类别、审计可靠性和下一步动作 |
