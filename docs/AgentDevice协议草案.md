...
| 问题 | v1 决议 |
|------|---------|
| 协议边界是否过大 | v1 按 Core Protocol、Governance Profile、Control Transfer Profile、Device Profile 分层；Core Protocol 只保留最小互操作内核 |
| `Device Context` 是否足够准确 | 足够。v1 不进一步拆分 `context_id` 和 `owner_scope` |
| `Agent Device` 是否会被误解为 Agent 私有设备 | 保留命名，并在摘要中明确它不是“Agent 的设备” |
| 是否定义统一 request/response schema | 不定义。v1 定义 Agent-readable semantic contract，允许目标平台通过 CLI、Tool、Skill、MCP、HTTP API、SDK 或其他入口表达同一组语义 |
| 是否定义统一错误模型 | 不定义统一错误码或错误返回结构，也不作为后续版本方向。通用协议层只要求最小失败语义，确保 Agent 能判断执行状态、副作用状态、重试安全性、失败类别和审计可靠性；这些维度允许表达未知、未确认或无法分类，下一步动作只在可判断时 SHOULD 提供 |
| 是否定义统一幂等模型 | 不定义统一幂等模型，也不要求所有动作可重试。治理类动作 SHOULD 尽量具备重复调用安全性；业务副作用动作必须通过重试安全性表达是否可重试或未知 |
| 是否定义 fencing token / heartbeat / renew API | 不定义统一模型。v1 只定义租约有效性边界：旧 lease 失效后不得继续授权新的外部副作用；具体阻断机制由平台或 Device Profile 实现 |
| 状态转换边界 | v1 规定核心状态、动作和状态约束；具体状态转移路径与失败落点由实现或 Device Profile 约束，并通过 Agent-readable semantic contract 表达状态变化和失败语义 |
| 状态发现模型 | v1 采用 query-first（查询优先）模型，要求 `DeviceVisibility` 可查询；不要求通用事件协议、Push state stream、webhook、SSE、WebSocket 或 queue |
| 能力发现与版本协商 | v1 不定义通用版本协商协议。Device Provider 必须声明 `Device Type`、`Device Profile` / 合规级别和关键能力边界；能力不可用时通过 Agent-readable semantic contract 表达不可用原因或降级语义 |
| `current_operator` 边界 | `current_operator` 只表示当前可产生外部副作用的控制者；观察、监督、审批和审计权限由 Device Context 策略分别授权，不进入 v1 角色模型 |
| 证据和现场引用 | `observable_surface_ref` 和 `evidence_refs` 必须是受治理引用。若引用因权限、保留或脱敏策略不可访问，返回语义必须表达受限、已过期、已脱敏或不可用，不能让 Agent 误以为证据完整可见 |
...
| v0.13 | 按 Core Protocol、Governance Profile、Control Transfer Profile 和 Device Profile 重组协议边界 |
| v0.14 | 补充 Agent-readable semantic contract，明确不定义统一 request/response schema |
| v0.15 | 补充最小失败语义，明确不定义统一错误码或错误返回结构 |
| v0.16 | 将不定义统一错误模型从 v1 取舍提升为协议原则 |
| v0.17 | 将下一步动作从 MUST 降为 SHOULD，避免要求设备编造建议 |
| v0.18 | 明确最小失败语义的 MUST 可表达未知，重试安全性不要求设备给出确定建议 |
| v0.19 | 将幂等和重试收敛到失败语义，不定义统一幂等模型 |
| v0.20 | 补充租约有效性边界，不定义统一 fencing token、heartbeat 或 renew API |
| v0.21 | 明确 v1 不定义完整状态转换表，仅约束核心状态、动作和语义表达 |
| v0.22 | 将状态转换决议改为边界陈述，去除问答式表述 |
| v0.23 | 改为 query-first 状态发现模型，不要求事件协议或状态推送流 |
| v0.24 | 补充能力发现与拒绝语义，不定义通用版本协商协议 |
| v0.25 | 明确不要求 Agent 操作前单独校验 lease，也不定义统一 token 或凭证格式 |
| v0.26 | 明确成功和失败判断必须绑定语义维度，审计状态不能被动作执行状态覆盖 |
| v0.27 | 明确 current_operator 只约束外部副作用控制者，不定义完整角色模型 |
| v0.28 | 明确 observable_surface_ref 和 evidence_refs 必须是受治理引用 |
| v0.29 | 明确合规判断基于暴露给 Agent 的 contract surface |
