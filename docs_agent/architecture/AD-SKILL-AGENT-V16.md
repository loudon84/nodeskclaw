...
target_branch: main
review_verdict: PASS
approved_at: 2026-09-01T09:26:59+08:00
source_revision: user-input:2026-09-01/contract-first-external-work-boundary
...
采用 Option B 与 Option E（方案 B/E）。v1.6 保持一个由独立 Stage PRD 组成的交付系列；v1.1.0 修订在 RM-01 至 RM-04 之后增加 RM-05 至 RM-10，不回写既有阶段语义。所有阶段保留 Backend=Control Plane（控制面）、Agent=Execution Plane（执行面）的既有架构，不新增第三个服务、第二个 Run 终态 Owner 或仓内 Work 前端 Owner。
...
| 外部 Work Skill-first Consumer Contract | `nodeskclaw-backend` Skill Run Contract Package | 本仓库发布版本化合同与 Backend 一致性证据；外部 Work 是仓外 Consumer，不属于实施范围 |
...
| 把外部 Work 发布结果误作本仓库 DONE 条件 | RM-09 只验证合同制品与 Backend conformance（符合性），仓外适配不进入本仓实施证据 | Roadmap/Plan 出现外部前端文件、构建或发布 Todo 时退回上游修订 |
