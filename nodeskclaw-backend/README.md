...
绑定实例在 `advanced_config` 中写入完整 Docker 接管信息（`paths` / `compose` / `webui` / `capabilities`），默认 `lifecycle_mode=managed_compose`。删除或解除关联时仅软删除数据库记录，不执行 `docker compose down` / `docker rm`，也不删除宿主机目录。

相关环境变量：

| 变量 | 说明 |
|------|------|
| `DOCKER_PUBLIC_SCHEME` | WebUI 公共访问协议，默认 `http` |
| `DOCKER_PUBLIC_HOST` | WebUI 公共访问主机；未设置时尝试从 `PORTAL_BASE_URL` 推导 |
| `DOCKER_COMPOSE_FILE` | 全局 compose 文件路径（关联扫描 fallback） |
| `DOCKER_ATTACH_SCAN_DIRS` | 扫描已有实例目录，逗号分隔 |
| `HERMES_INSTANCES_ROOT` | Hermes 外部实例目录根路径；默认取 `DOCKER_ATTACH_SCAN_DIRS` 首项 |
| `HERMES_DEFAULT_GATEWAY_INTERNAL_PORT` | 容器内 Gateway 端口，默认 `8642` |
| `HERMES_AGENT_HOST_IP` | Agent Gateway/WebUI 公共访问 IP；未设置时回落 `DOCKER_PUBLIC_HOST` |
| `HERMES_GATEWAY_PROBE_TIMEOUT_SECONDS` | Gateway 探活超时（秒），默认 `3` |
| `HERMES_GATEWAY_PROBE_CONCURRENCY` | 批量探活并发上限，默认 `5` |
| `HERMES_AUTO_PROBE_AFTER_SCAN` | 扫描绑定后是否自动探活，默认 `true` |

实例 `.env` 需配置 `HERMES_GATEWAY_PORT`（宿主机映射端口）与 `HERMES_WEBUI_PORT`。绑定后 `advanced_config` 会写入 `gateway_url` / `hermes_base_url`，供 Hermes MCP 与 Desktop Client 调用 Gateway Runtime。

**Hermes Agent 绑定 API（v4.4）** — 前缀 `/api/v1/hermes`：

| 方法 | 路径 | 权限 | 说明 |
|------|------|------|------|
| POST | `/agents/scan-existing` | `hermes_agent:manage` | 扫描 `HERMES_INSTANCES_ROOT` 下实例目录并 upsert |
| GET | `/agents` | `hermes_agent:view` | 列出已绑定 Agent（默认 `include_unbound=false`；支持 `dispatchable_only=true` 仅返回可下发实例） |

**Hermes 模块数据边界（v4.5.3）** — `HermesBoundAgentScopeService`（`app/services/hermes_external/hermes_bound_agent_scope_service.py`）为 Hermes 业务默认数据源：Runtime、Diagnostics、Tasks、Queue、Metrics、Copilot Desktop（`GET /hermes/client/agents`）默认只暴露已绑定 `external_docker` AI 员工。未绑定 Docker 容器仅可通过 `POST /hermes/agents/scan-existing`、`GET /hermes/agents?include_unbound=true`（需 `hermes_agent:manage`）或 `GET /docker/attachable-containers` 在绑定流程中出现。

| GET | `/agents/{profile_name}` | `hermes_agent:view` | 单实例详情 |
| POST | `/agents/{profile_name}/probe` | `hermes_agent:manage` | 单实例 Gateway 探活 |
| POST | `/agents/probe-all` | `hermes_agent:manage` | 批量探活并返回状态统计 |
| GET | `/agents/{profile_name}/diagnostics` | `hermes_agent:view` | 实例诊断检查项 |
| GET | `/agents/{profile_name}/insight` | `hermes_agent:view` | Hermes Insight 运行统计（固定最近 30 天，支持 `profile=all` 或单 profile） |

**Hermes Insight（v5.0）** — 从各 profile 独立 `HERMES_HOME` 下的 `state.db` 与 `webui/sessions/_index.json` 只读采集用量统计，并汇总 Docker 容器与 profile 运行时状态。不依赖 Hermes API Server 8642 或 hermes-webui 服务。实现位于 `app/services/hermes_external/insight/`。

治理 API `GET /hermes/agents/runtime`（v4.2）保持不变；Portal「Hermes Agent」页数据源改为 `GET /hermes/agents`。


| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/v1/hermes-experts/instances/{id}/sync-status` | 同步 Docker 容器与健康状态 |
| POST | `/api/v1/hermes-experts/instances/{id}/actions/detach` | 解除外部关联（不删容器/目录） |
