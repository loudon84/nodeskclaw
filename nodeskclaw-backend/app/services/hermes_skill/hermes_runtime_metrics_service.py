...
class HermesRuntimeMetricsService:
    ...
    async def get_metrics(self, org_id: str, range_key: str = "7d") -> dict:
        ...
        overview = await self._overview(org_id, since)
        failed_agents = await self._top_failed_agents(org_id, since)
        failed_skills = await self._top_failed_skills(org_id, since)
    ...
    async def _overview(self, org_id: str, since: datetime) -> dict:
    ...
    async def _top_failed_agents(self, org_id: str, since: datetime, limit: int = 10) -> list[dict]:
        stmt = (
            ...
            .where(
                ...
                HermesTask.created_at >= since,
                HermesTask.status.in_([TaskStatus.FAILED, TaskStatus.TIMEOUT]),
                HermesTask.agent_id.isnot(None),
            )
    ...
    async def _top_failed_skills(self, org_id: str, since: datetime, limit: int = 10) -> list[dict]:
        stmt = (
