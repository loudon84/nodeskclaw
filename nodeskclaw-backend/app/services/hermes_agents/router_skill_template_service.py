...
def render_router_skill_md(mcp_name: str, tools: list[dict[str, Any]]) -> str:
    ...
    content = (
        ...
        "## 异步任务等待规则\n\n"
        "远程 MCP business skill 默认采用事件驱动模式（async_event）。\n\n"
        "当业务 tool 返回 `execution_mode=async_event` 时：\n\n"
        "- 禁止再次调用原业务 tool。\n"
        "- 应订阅返回的 `event_stream`（SSE），等待 `task.completed` 事件。\n"
        "- SSE 不可用时，只能使用 `wait_strategy.poll_tool`（通常为 nodeskclaw_task_wait）继续等待。\n\n"
