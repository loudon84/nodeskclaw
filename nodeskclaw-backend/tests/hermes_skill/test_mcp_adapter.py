...
async def test_compute_output_dir_relative():
    ...
    with patch.object(adapter, "_get_instance", return_value=instance):
...
async def test_compute_output_dir_absolute():
    ...
    with patch.object(adapter, "_get_instance", return_value=instance), \
         patch("app.services.hermes_skill.hermes_agent_adapter.ArtifactService") as mock_artifact_svc:
...
async def test_get_run_status():
    ...
    with patch.object(adapter, "get_run", return_value={"status": "completed"}):
